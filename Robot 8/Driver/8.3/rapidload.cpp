...
*/
  ...
  frontRightMetro.spin(reverse, 100, percent);
  while (true) {
    if ((rpLoad.objectDistance(inches) > 28 && rpLoad.objectDistance(inches) < 31.5) && !backrollerSensor.isNearObject()) {
      leftDrive.spin(forward);
      rightDrive.spin(forward);
      leftDrive.resetPosition();
      rightDrive.resetPosition();

      while (leftDrive.position(degrees) < 69) wait(20, msec);
    ...
    else if ((rpLoad.objectDistance(inches) >= 31.5 && rpLoad.objectDistance(inches) < 36) && !backrollerSensor.isNearObject()) {
      leftDrive.spin(forward);
      rightDrive.spin(forward);

      while (rpLoad.objectDistance(inches) > 28) wait(20, msec);
